...
STUDENT ACTIVITY
	...
	5.	Combining character sets can create sequences of matches.
		...
		Answer: 1 or more letters at the beginning and 1 # at the end exp: mike1 but not mike11 or 11mike
...
*/
	...
	console.log(str.search(emailRegEx)); //begininning index of where first email starts
	
	var clean =	str.replace(emailRegEx,"xxx@xxx.com");//replace first email found
	console.log(clean);
	//or
	var clean =	str.replace(/(\w[-._\w]*\w@\w[-._\w]*\w\.\w{2,3})/,"xxx@xxx.com");
	console.log(clean);
	
	var clean = clean.replace(emailRegEx, "jc@gmail.com");//replace first email found
	console.log(clean);
...
*/
	...
	//console.log(document.getElementById('tagbox').innerHTML = Math.random());//random # will always be between 0 and 1
	//var rand = (Math.random() * 10 +1)//random # between 1 and 100
	var rand = (Math.random() * 10 +1);//random # between 1 and 10 first is max and last is min
	console.log('rand: ', rand);
	
	var num = Math.floor(rand);//round to nearest #
	console.log(num);
